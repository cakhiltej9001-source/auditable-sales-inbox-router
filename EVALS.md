...
`eval/labels.json` contains 60 manually reviewed deterministic routing cases. `eval/challenge_cases.json` adds nine harder worked-example regressions for Indian comma currency, ordinal and Hinglish dates, explicitly priced sponsorships, government overrides, ambiguous requests, and directional vendor spam.
...
| marketing | 9 | 1.000 | 1.000 |
| alliances | 9 | 1.000 | 1.000 |
| finance | 9 | 1.000 | 1.000 |
| triage | 9 | 1.000 | 1.000 |
| out_of_office | 4 | 1.000 | 1.000 |
| newsletter | 4 | 1.000 | 1.000 |
| vendor_spam | 5 | 1.000 | 1.000 |

These 69 results measure a transparent rule-regression corpus, not unseen generalization. GitHub CI reruns the evaluator on every push and fails if output differs from the labels.
...
3. **Implicit company names:** explicit `Company:` labels are extracted, but the fallback deliberately avoids deriving a company from an email domain. Some genuine company values therefore remain null.
